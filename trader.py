...
#  EDGE THRESHOLDS (FIXED) 
# IMPORTANT: Correct key name + safe looseness

BASE_EDGE_THRESHOLD: float = max(
    0.0015,
    float(CONFIG.get("BASE_EDGE_THRESHOLD", 0.0025))
)

LOW_LIQ_EDGE: float = max(
    0.0012,
    float(CONFIG.get("MIN_EDGE_LOW_LIQUIDITY", 0.0018))
)
...
EXPLORATION_RATE: float = min(
    0.40, max(0.10, float(CONFIG.get("EXPLORATION_RATE", 0.35)))
)

EXPLORATION_SIZE: float = max(
    1.1, float(CONFIG.get("EXPLORATION_SIZE", 1.35))
)
...
TRADE_COOLDOWN: float = max(
    30.0, float(CONFIG.get("TRADE_COOLDOWN", 45.0))
)
